<?php 
	trait ProductsModel{
		...
		public function modelReadField($recordPerPageField){
			$fields_id =isset($_GET["id"]) ? $_GET["id"] : 0;
			//lay bien page truyen tu url
			$page = isset($_GET["p"])&& $_GET["p"] > 0 ? $_GET["p"]-1 : 0;
			//lay tu ban ghi nao
			$from = $page * $recordPerPageField;
			//---
			$sqlOrder = "";
			$order = isset($_GET["order"]) ? $_GET["order"] : "";
			switch ($order) {
			...
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->query("select * from products where fields_id = $fields_id $sqlOrder limit $from, $recordPerPageField");
			//tra ve nhieu ban ghi
			return $query->fetchAll();
		}
		...
		//tinh tong so ban ghi
		public function modelTotalRecordField(){
			$fields_id =isset($_GET["id"]) ? $_GET["id"] : 0;
			//lay bien ket noi csdl
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->query("select * from products where fields_id = $fields_id");
			//tra ve so luong ban ghi
			return $query->rowCount();
		}
		...
		public function modelGetField($fields_id){			
			//lay bien ket noi csdl
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->query("select * from fields where id = $fields_id");
			//tra ve so luong ban ghi
			return $query->fetch();
		}
